...
public class KoreMovingDouble
{
    ...
    public double CurrentValue
    {
        get
        {
            lock (_sync)
            {
                return GetValueForTimeInternal(KoreCentralTime.RuntimeSecs, true);
            }
        }
    }
    ...
    public double GetValueForTime(double runtimeSecs)
    {
        lock (_sync)
        {
            return GetValueForTimeInternal(runtimeSecs, false);
        }
    }

    private double GetValueForTimeInternal(double runtimeSecs, bool mutateWhenSettled)
    {
        double elapsedSecs = runtimeSecs - StartRuntimeSecs;
        if (elapsedSecs >= ClockDurationSecs)
        {
            if (mutateWhenSettled)
            {
                StartValue        = TargetValue;
                StartRuntimeSecs  = runtimeSecs;
                ClockDurationSecs = 0d;
            }
